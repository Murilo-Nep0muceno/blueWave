...
public class InscricaoController {
    ...
    public ModelAndView verMinhasInscricoes(HttpSession session) {
        ...
        try {
            ...
            // Separar por status com base na data atual E status da vaga
            ...
            List<Inscricao> vagasAtivas = inscricoes.stream()
                    .filter(inscricao -> {
                        ...
                        // Considera ativa se:
                        // 1. O status da vaga é ATIVA
                        // 2. A data é hoje ou futura
                        return vaga.getData() != null &&
                                vaga.getStatus() == Vagas.StatusVaga.ATIVA &&
                                !vaga.getData().isBefore(hoje);
                    })
                    .collect(Collectors.toList());
            ...
            List<Inscricao> vagasEncerradas = inscricoes.stream()
                    .filter(inscricao -> {
                        Vagas vaga = inscricao.getVaga();
                        // Considera encerrada se:
                        // 1. O status da vaga NÃO é ATIVA (CONCLUIDA ou INTERROMPIDA)
                        // OU
                        // 2. A data já passou (mesmo que o status seja ATIVA)
                        return vaga.getData() != null && (
                                vaga.getStatus() != Vagas.StatusVaga.ATIVA ||
                                        vaga.getData().isBefore(hoje)
                        );
            ...
            // Debug para verificar os números
            System.out.println("=== DEBUG MINHAS INSCRIÇÕES (CORRIGIDO) ===");
            System.out.println("Total de inscrições: " + inscricoes.size());
            System.out.println("Vagas ativas: " + vagasAtivas.size());
            System.out.println("Vagas encerradas: " + vagasEncerradas.size());
            System.out.println("Data atual: " + hoje);

            for (Inscricao inscricao : inscricoes) {
                Vagas vaga = inscricao.getVaga();
                boolean isStatusAtiva = vaga.getStatus() == Vagas.StatusVaga.ATIVA;
                boolean isDataFutura = !vaga.getData().isBefore(hoje);
                boolean isAtiva = isStatusAtiva && isDataFutura;

                System.out.println("Vaga: " + vaga.getNome() +
                        " | Data: " + vaga.getData() +
                        " | Status: " + vaga.getStatus() +
                        " | Status é ATIVA: " + isStatusAtiva +
                        " | Data é futura/hoje: " + isDataFutura +
                        " | Considerada ATIVA: " + isAtiva);
            }
            System.out.println("===============================");
